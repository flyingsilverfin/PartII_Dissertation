...
\end{lstlisting}
	...
	\end{figure}
		...
		A traditional vector clock is outlined is Appendix [TODO]. In this system, the causal delivery layer must delay messages that arrive before all of their causal dependencies are delivered, which requires calculating exactly which messages are missing.
		
		To this end, I modified the rule [TODO rule number from appendix] to only increment the local clock whenever a message is generated. This way, each value in a vector represents exactly how many messages have been sent by the corresponding client. 
