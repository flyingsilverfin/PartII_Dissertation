...
\end{lstlisting}
			...
			TODO
			talk about cost (minimal) and the fact that an undo that happened a lot time ago and is no longer on an operation stack has the same weight as one that happened more recently and is still in a stack(by $\rightarrow$) ??? 
