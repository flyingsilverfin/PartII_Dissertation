...
\end{lstlisting}
	...
	\end{figure}
		...
		A traditional vector clock is outlined is Appendix \ref{appendix:vectorclocks}. In this system, the causal delivery layer must delay messages that arrive before all of their causal dependencies are delivered, which requires calculating exactly which messages are missing. To this end, I modified the vector merge rule (number 4 from the appendix) to only increment the local clock whenever a message is generated. This way, each value in a vector represents exactly how many messages are known to have been sent by the corresponding client. 
...
\end{lstlisting}
		...
		CCT Undo requires users agree to make deleted characters reappear. It would be preferable for any user to be able to immediately undo a character regardless of other users.
			...
			\textcolor{blue}{This could be moved into the appendix to save word count ~320 words less\\}
			We only need to consider commutativity with other operations that may operate on the same data in the CRDT and therefore conflict. Since any given delete, undo, or redo operation $op$ only affects the specific node $node$ in the linked list identified by $op.deleteId$, the only conflicting operations might be other delete, undo delete, and redo delete operations.
		...
		\subsection{ShareJS Latencies}
			...
			%\resizebox {\columnwidth} {!}{
				\begin{tikzpicture}
					...
					\addplot+[blue] table[x index=0, y index =1] {data/latencies.dat_HIST.dat};
					\addlegendentry{10 Requests per second}
					\addplot+[red] table[x index=0, y index =2] {data/latencies.dat_HIST.dat};
					\addlegendentry{20 Requests per second}
					\addplot+[green] table[x index=0, y index =3] {data/latencies.dat_HIST.dat};
					\addlegendentry{50 Requests per second}
					\addplot+[purple] table[x index=0, y index =4] {data/latencies.dat_HIST.dat};
					\addlegendentry{100 Requests per second}
					\addplot+[black] table[x index=0, y index =5] {data/latencies.dat_HIST.dat};
			...
			At increasing server load, the distribution becomes approximately normally distributed. The mean shifts is approximately 29ms when operating at 200 requests per second from 10 separate TCP connections. This increase likely corresponds to load incurred either on the server itself or on the operating system network stack which handles locally routed packets. For experiments on the CRDT system, 30ms will be the mean latency with a standard deviation of 10 ms.
			
			Interestingly, as shown Figure \ref{fig:localping} executing a local ping takes on average 0.03 ms, rather than the 3 to 30 ms supported by the data above. The discrepancy might be attributed to the nature of the Node server used, ShareJS, Chrome and the use of HTTP requests, interaction with logging, or operating system effects such as context switching. For the purpose of this project, the actual latencies are not very important, and indeed higher values make it easier to simulate concurrency.
			
			\begin{figure}[htb]
			\centering
			\includegraphics[width=1\linewidth]{figs/local_ping.png}
			\caption[Local Ping]{Execution of \textit{ping} to localhost.}
			\label{fig:localping}
			\end{figure}
			
		\subsection{Memory}
		
			\paragraph{ShareJS Expectations}
				As noted in \cref{sec:sharejs} I expect both the server and clients to buffer past operations to transform concurrent operations against. To confirm this, I created a series of experiments which insert and remove the same character a number of times. This ensures the document size is not growing, so the only potential growth is storing past operations.
				
				Client memory use Figure [TODO]. next to server use figure [TODO]
		
			\paragraph{Single Client}
				I noted in \cref{sec:sharejs} a ShareJS client might scale linearly in terms of memory consumption due to saved operations and document size. Comparatively, a CRDT with $n$ characters must store $n$ links of constant size (ignoring immediate undo requirements), each identified by an identifier of decimal length $log(n)$, so one might expect a $O(nlog(n))$ space complexity. To test this, a series of experiments were designed using only a single client. Operations inserted only one character at a time. 
				[For the CRDT system, run this on both an Undo-enabled system, CCI Undo, and the Immediate Undo system. Plot all of these on the same graph.]
				
				\begin{figure}
				  \centering
				  \includestandalone[mode=buildnew]{data/1client_plot}
				  \caption[Single Client Memory Consumption] {Memory consumption of both systems with only one client. Interestingly, the server memory use decreases up to about 1000 insertions... [Explanation TODO]. All three components scale linearly with the number of insertions. It appears the predicted $log(n)$ factor has little effect at this scale of operation. In the plot, the error in the measurement of the CRDT client is too small to see.}
				\end{figure}
				
				Instead, a more reasonable interaction that might be expected is word-insertion rather than single character insertion. The performance of the CRDT should be identical, whereas ShareJS's should improve: fewer packets sent means fewer packets need to be saved client side, though document size is similar. 
				[For the CRDT system, run this on both an Undo-enabled system, CCI Undo, and the Immediate Undo system. Plot all of these on the same graph.]
			...
			\paragraph{Multiple Clients}
				Try inserting a set number (5000) characters into the document in both systems, but with split across increasing numbers of clients. 1 Client has already been done above. The insertions were split across 1, 2, 5, 10, 50, and 100 clients.
				[For the CRDT system, run this on both an Undo-enabled system, CCI Undo, and the Immediate Undo system. Plot all of these on the same graph.]
...
%		\paragraph{Vector Clocks} \label{sec:vectorclock}
