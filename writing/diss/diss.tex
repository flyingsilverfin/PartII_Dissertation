...
\section{CRDT-based system}
	...
	\subsection{CRDT}
		...
		The text CRDT supports two core operations: insert and delete.
			...
			An insert bundle \textit{B} contains
			\begin{itemize}
				\item A unique identifier  \textit{id} for the character
				\item The character \textit{c} itself
				\item The node after which to insert the character \textit{after}. This corresponds to the position in the linked list at which the character needs to be spliced into and another unique identifier.
			...
			\begin{enumerate}
				...
				\item Insert into the table \textit{B.id} mapped to a new node \textit{N} containing the character \textit{B.c} and a pointer to \textit{Prev}'s \textit{next} pointer
				\item Rewrite \textit{Prev}'s \textit{next} pointer to point to \textit{B}
			\end{enumerate}
			This is the standard procedure to insert a new node into a linked list.
			
			\subsubsection{Delete}
			Deleting a character from the text document generates a delete operation, which is transmitted as a bundle \textit{B} containing
			\begin{itemize}
			...
			\begin{enumerate}
				\item Locate the node \textit{N} corresponding to \textit{B.deleteId}
				\item Set a boolean flag in \textit{N.d} to true
			\end{enumerate}
